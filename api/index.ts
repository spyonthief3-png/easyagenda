...
import { users, locations, roomTypes, rooms, bookings, periods, holidays, blackouts, issueReports, auditLogs } from '../server/db/schema';
import { eq, and, sql } from 'drizzle-orm';
import { createId } from '@paralleldrive/cuid2';
import { formatToYyyyMmDd, addDays } from '../server/utils/date';
import { validateBody, loginSchema } from '../server/validations';
...
const ensureMaintenanceOrAdmin = (req: any, res: any, next: any) => {
...
app.get('/api/availability', authenticate, async (req: any, res) => {
    ...
    try {
        ...
        const allRooms = await db.query.rooms.findMany({ where: eq(rooms.isActive, true) });
        ...
        const holiday = await db.query.holidays.findFirst({ where: eq(holidays.date, dateStr) });
        if (holiday) {
            const result: any = {};
            for (const room of allRooms) {
                result[room.id] = {};
                for (const period of allPeriods) {
                    result[room.id][period.code] = { status: 'HOLIDAY', holiday: { name: holiday.name } };
                }
            }
            res.json(result);
            return;
        ...
        const dayBlackouts = await db.query.blackouts.findMany({ where: eq(blackouts.date, dateStr) });
        ...
        for (const room of allRooms) {
            ...
            for (const period of allPeriods) {
                ...
                if (blackout) {
                    result[room.id][period.code] = { status: 'BLACKOUT', blackout: { reason: blackout.reason } };
                    continue;
                }

                const booking = dayBookings.find(b => b.roomId === room.id && b.periodId === period.id);
                if (booking) {
                    result[room.id][period.code] = {
                        status: booking.userId === currentUserId ? 'MY_BOOKING' : 'BOOKED',
                        booking: { id: booking.id, title: booking.title, user: { name: booking.user.name } }
                    };
                } else {
...
app.get('/api/bookings', authenticate, async (req, res) => {
    try {
        const allBookings = await db.query.bookings.findMany({
            with: { room: { with: { location: true, roomType: true } }, user: true, period: true },
...
app.get('/api/bookings/my', authenticate, async (req: any, res) => {
    try {
        const myBookings = await db.query.bookings.findMany({
            where: and(eq(bookings.userId, req.user.id), eq(bookings.status, 'CONFIRMED')),
            with: { room: { with: { location: true, roomType: true } }, user: true, period: true },
            orderBy: (bookings, { desc }) => [desc(bookings.date)]
        });
        res.json(myBookings);
    } catch (e) { res.status(500).json({ error: 'Failed to fetch my bookings' }); }
});
...
app.post('/api/blackouts/range', authenticate, ensureMaintenanceOrAdmin, async (req, res) => {
    try {
        ...
        if (newBlackouts.length > 0) await db.insert(blackouts).values(newBlackouts);
...
app.get('/api/dashboard', authenticate, async (req, res) => {
    try {
        ...
        const upcomingBookings = allBookings.filter(b => b.date >= todayStr).length;
        ...
        allBookings.forEach(b => {
            byLocation[b.room.location.name] = (byLocation[b.room.location.name] || 0) + 1;
            byRoomType[b.room.roomType.name] = (byRoomType[b.room.roomType.name] || 0) + 1;
            byRoom[b.room.name] = (byRoom[b.room.name] || 0) + 1;
            byUser[b.user.name] = (byUser[b.user.name] || 0) + 1;
        ...
        res.json({
            ...
            bookingsByDay: formatStat(byDay).sort((a, b) => a.label.localeCompare(b.label)),
        });
    } catch (e) {
